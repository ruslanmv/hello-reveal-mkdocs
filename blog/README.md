...
## Level 3: Exporting to PDF (The Fixed Version!)
...
### Problem: "Printed 1 slides" instead of all slides

**Solution:** This is exactly what we fixed! Make sure you're using:
- `--load-pause 8000` (not 1500)
- `--pause 2000` (must be present)
- `--slides 1-100` (not `1-`)
- No `?print-pdf` in URL

### Problem: PDF is blank or has wrong styling

**Symptoms:** PDF generates but looks wrong

**Solution:**
1. Verify your HTML slides load correctly in a browser first
2. Make sure you're using CDN URLs (not local `node_modules`)
3. Try increasing timing: `LOAD_PAUSE=12000 PAUSE=3000 make pdf`

### Problem: "File css/reset.css not found"

**Symptoms:** Pandoc can't find Reveal.js assets

**Solution:** Your Pandoc version doesn't match the Reveal.js version. Our `generate_slides.sh` script fixes this automatically by detecting your Pandoc version.

### Problem: Assets 404 in GitHub Actions

**Symptoms:** Slides work locally but break in CI

**Solution:** Always use CDN URLs, never local paths. The script already does this.

### Problem: MkDocs can't find slides

**Symptoms:** Navigation links are broken

**Solution:**
1. Verify files exist: `ls -la docs/slides/`
2. Check `mkdocs.yml` paths match exactly
3. Run `make slides pdf` before `make serve`
...
✅ **Single source of truth** — One Markdown file  
✅ **Beautiful documentation** — Professional MkDocs site  
✅ **Interactive presentations** — Reveal.js HTML with all features  
✅ **Portable PDFs** — High-quality decks for offline use  
✅ **Automated deployment** — Push to main, site updates automatically  
✅ **Version control** — Full history of all changes  
✅ **Zero manual sync** — No more drift between docs and slides  
